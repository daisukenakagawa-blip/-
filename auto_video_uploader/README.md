...
`AUTO_TOPIC=1`(クラウド実行では既定で有効)にすると、**テーマの在庫が無くても止まりません**。

```
毎日 08:00 JST に自動起動
  → シートに pending があればそれを処理
  → 無ければ AI(ANTHROPIC_API_KEY 設定時)または内蔵ネタ帳から新テーマを自動生成
  → 台本 → 音声 → 動画 → 品質チェック → 19:00 に予約投稿
```

- 生成されたテーマは topics.csv に記録されるため、過去ネタとの重複は自動で回避されます
- 自分でネタを入れたいときは今まで通りシートに行を足すだけ(そちらが優先されます)
- ローカル実行では `python main.py --auto-topic` でも同じ動作になります

### 放置運用を止めないためのチェックリスト(重要)

完全放置には以下が満たされている必要があります。**どれか1つでも欠けると静かに止まります。**

| 項目 | 放置で壊れる原因 | 対策 |
|---|---|---|
| OAuth 同意画面が「本番環境」 | テスト状態だと token が**7日で失効** | Google Cloud Console で「アプリを公開」 |
| ワークフローがデフォルトブランチにある | schedule はデフォルトブランチでしか動かない | このブランチをマージする |
| GitHub の通知メール | 失敗に気付けない | 失敗時のメールだけは確認する |
| YouTube API クォータ | 1日1本なら余裕(1本=約1,600/10,000) | 1日1本運用のままにする |

### 収益についての正直な話

このツールが自動化するのは「動画を作って投稿し続けること」であり、**収益の発生そのものは自動化できません**。

- YouTube の収益化には **登録者1,000人 + (Shorts) 90日間で1,000万回再生** などの条件があります
- 収益化後も収入額はコンテンツの伸び次第で、保証はありません
- 完全放置でも週1回程度はアナリティクスを見て、伸びたネタの傾向をシートに反映すると成長が段違いに速くなります(ここだけは人間がやる価値があります)
