...
## LINE通知(投稿が終わったらLINEに通知)

`.env` に `LINE_CHANNEL_ACCESS_TOKEN` を設定すると、**投稿完了時に自分のLINEへ通知**が
届きます(未設定ならスキップ)。設定手順は `note_writer/README.md` の
「LINE通知の設定」と同じトークンが使えます(同じ公式アカウントでOK)。
